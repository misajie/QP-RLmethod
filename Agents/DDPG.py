...
class DDPGAgent:
    def __init__(self, env,
        ...
        self.noise = Normal(noise_mu * torch.ones(action_dim), noise_gamma * torch.ones(action_dim))
    ...
    #     start = time.time()
    #     with torch.no_grad():
    #         with self.create_pool(4) as pool:
    #             scores, niters, results = [], [], []
    #             for i in range(test_no):
    #                 results.append(pool.apply_async(self.run_episode, (),{}))
    #             pool.close()
    #             pool.join()
    #             for r in results:
    #                 s, n = r.get()
